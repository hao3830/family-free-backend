...
CREATE TRIGGER new_member_trigger BEFORE INSERT ON THANHVIEN
FOR EACH ROW
BEGIN
    DECLARE thehe_value INT;
    
    IF NEW.MATHANHVIENCU IS NOT NULL AND NEW.MALOAIQUANHE = 'QH001' THEN
        SET thehe_value = (SELECT THEHE + 1 FROM THANHVIEN WHERE MATHANHVIEN = NEW.MATHANHVIENCU);
    ELSEIF NEW.MATHANHVIENCU IS NOT NULL AND NEW.MALOAIQUANHE = 'QH002' THEN
        SET thehe_value = (SELECT THEHE FROM THANHVIEN WHERE MATHANHVIEN = NEW.MATHANHVIENCU);
    ELSE
        SET thehe_value = 0;
    END IF;
    
    SET NEW.THEHE = thehe_value;
END//
