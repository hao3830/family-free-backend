...
class ErrorList(object):
    ...
    @dispatch(str)
    def __call__(self, code):  # noqa: F811
        if code.isdigit():
            code = int(code)
            if code not in self.int_list:
                logger.error(f"Index out of range: {code}")
                return {
                    **self.str_list["IndexOutOfRange"].get_print(),
                    **{"message": f"Code {code} is not exists in error list"},
                }
            return self.int_list[code].get_print()
...
c400 = ErrorCode(int_code=400, str_code="BadRequest", message="Bad Request")
rcode.add_error(c400)
...
c601 = ErrorCode(
    int_code=601, str_code="FaceNotFound", message="Face Not Found"
)
rcode.add_error(c601)

c604 = ErrorCode(int_code=604, str_code="LoginFailed", message="Login Failed")
rcode.add_error(c604)

c605 = ErrorCode(
    int_code=605, str_code="LoginSuccess", message="Login Successful"
)
rcode.add_error(c605)

c606 = ErrorCode(
    int_code=606, str_code="PermissionDenied", message="Permission Denied"
)
rcode.add_error(c606)

c607 = ErrorCode(
    int_code=607, str_code="SignatureExpired", message="Signature Expired"
)
rcode.add_error(c607)

c608 = ErrorCode(
    int_code=608, str_code="BadSignature", message="Bad Signature"
)
...
c610 = ErrorCode(
    int_code=610,
    str_code="UserNameNotFound",
    message="UserName/UserID Not Found",
)
rcode.add_error(c610)

c613 = ErrorCode(
    int_code=613, str_code="IndexOutOfRange", message="Index Out Of Range"
)
rcode.add_error(c613)

c614 = ErrorCode(int_code=614, str_code="NotFound", message="Not Found")
rcode.add_error(c614)

c618 = ErrorCode(
    int_code=618, str_code="EncodeTokenFailed", message="Encode Token Failed"
)
rcode.add_error(c618)

c619 = ErrorCode(
    int_code=619, str_code="InvalidBearer", message="Invalid Bearer"
)
rcode.add_error(c619)

c620 = ErrorCode(
    int_code=620, str_code="UserNameExisted", message="UserName/Email Existed"
)
rcode.add_error(c620)

c621 = ErrorCode(
    int_code=621,
    str_code="MoreThanOneFaceInImage",
    message="More Than One Face In Image",
)
rcode.add_error(c621)

c714 = ErrorCode(
    int_code=714,
    str_code="CanNotReadImageFromBytes",
    message="Can Not Read Image From Bytes",
)
rcode.add_error(c714)

c801 = ErrorCode(
    int_code=801, str_code="StudentNotFound", message="Student Not Found"
)

rcode.add_error(c801)

c900 = ErrorCode(
    int_code=900, str_code="SQLExecuteError", message="SQL Execute Error"
)
...
c1201 = ErrorCode(
    int_code=1201, str_code="FileNotFound", message="File Not Found"
)
...
c1203 = ErrorCode(
    int_code=1203, str_code="ReadFileError", message="Read File Error"
)
rcode.add_error(c1203)

c1204 = ErrorCode(
    int_code=1204, str_code="WriteFileError", message="Write File Error"
)
rcode.add_error(c1204)

c1205 = ErrorCode(
    int_code=1205, str_code="DeleteFileError", message="Delete File Error"
)
rcode.add_error(c1205)

c1206 = ErrorCode(int_code=1206, str_code="BadZipFile", message="Bad Zip File")
rcode.add_error(c1206)

c1305 = ErrorCode(
    int_code=1305,
    str_code="CanNotParseJSONString",
...
c1306 = ErrorCode(
    int_code=1306,
    str_code="CanNotParseBase64StringToImage",
    message="Can Not Parse Base64 String To Image",
)
rcode.add_error(c1306)

c1401 = ErrorCode(
    int_code=1401,
    str_code="WrongUserPassword",
    message="Wrong Username or Password",
)
rcode.add_error(c1401)

c1402 = ErrorCode(
    int_code=1402, str_code="VerifyTokenFailed", message="Verify Token Failed"
)
...
c1601 = ErrorCode(
    int_code=1601,
    str_code="CallInternalServiceFailed",
    message="Call Internal Service Failed",
)
